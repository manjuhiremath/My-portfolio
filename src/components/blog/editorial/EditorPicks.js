...
export default function EditorPicks({ blogs }) {
  ...
  return (
    <section className="relative overflow-hidden rounded-xl bg-slate-50 dark:bg-slate-800/30 p-8 sm:p-10 lg:p-12 border border-slate-100 dark:border-slate-800/50 shadow-sm transition-all hover:shadow-md">
          ...
          return (
            <Link key={blog._id} href={href} className="group">
              <article className="space-y-5">
                {/* Image */}
                <div className="relative aspect-[4/3] overflow-hidden rounded-lg bg-slate-200 dark:bg-slate-700 shadow-sm transition-all duration-500">
                  <Image
                    src={imageUrl}
                    alt={blog.title}
                    fill
                ...
                <div className="space-y-3 px-2">
                  ...
                  <h3 className="line-clamp-2 text-sm font-black leading-snug text-slate-900 dark:text-white transition-colors">
                    {blog.title}
                  </h3>
                  <p className="line-clamp-2 text-[11px] text-slate-500 dark:text-slate-400 font-medium leading-relaxed">
