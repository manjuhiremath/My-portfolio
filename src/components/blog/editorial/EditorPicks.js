...
export default function EditorPicks({ blogs }) {
  ...
  return (
    <section className="rounded-2xl bg-gradient-to-br from-slate-50 to-slate-100 p-5 sm:p-6 lg:p-8">
      {/* Section Header */}
      <div className="mb-6 flex items-center gap-2.5">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500">Handpicked articles by our team</p>
        </div>
      </div>
          ...
          return (
            <Link key={blog._id} href={href} className="group">
              <article className="space-y-3">
                {/* Image */}
                <div className="relative aspect-[4/3] overflow-hidden rounded-xl bg-slate-200 shadow-sm transition-all group-hover:shadow-md">
                  <Image
                    src={imageUrl}
                    alt={blog.title}
                    fill
                ...
                <div className="space-y-2">
                  ...
                  <h3 className="line-clamp-2 text-base font-bold leading-snug text-slate-900 transition-colors group-hover:text-orange-600 sm:text-lg">
                    {blog.title}
                  </h3>
