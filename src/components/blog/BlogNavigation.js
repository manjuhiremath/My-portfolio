...
export default function BlogNavigation() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border-b border-gray-100 dark:border-gray-800 transition-all duration-500">
      <div className="mx-auto max-w-[1440px]">
            ...
            {categories.slice(0, 10).map((cat) => (
              <button
                ...
                className={`whitespace-nowrap rounded-lg px-2 py-1 text-[9px] font-semibold uppercase tracking-wide transition-all ${activeFilter === cat.name
                    ? 'bg-gray-900 dark:bg-white text-white dark:text-gray-900 shadow-sm'
                    : 'bg-gray-200 dark:bg-gray-800/50 text-gray-600 dark:text-gray-600 hover:bg-gray-400 dark:hover:bg-gray-800 hover:text-primary'
                  }`}
              >
                {cat.name}
              </button>
        ...
        {isSearchOpen && (
          <div className="border-t border-gray-100 dark:border-gray-800 bg-white/50 dark:bg-gray-900/50 px-3 py-3 lg:px-6 animate-in slide-in-from-top duration-300">
            <div className="mx-auto max-w-3xl">
              <form onSubmit={handleSearch} className="relative group">
                <FiSearch className="absolute left-3 top-1/2 h-4 w-4 -trangray-y-1/2 text-gray-400 group-focus-within:text-primary transition-colors" />
                <input
                  autoFocus
                  value={query}
                  onChange={(e) => setQuery(e.target.value)}
                  placeholder="Search articles, topics, or tags..."
                  className="h-9 w-full rounded-xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-800/50 pl-10 pr-4 text-xs dark:text-white font-medium outline-none transition-all focus:border-primary focus:ring-2 focus:ring-primary/5 shadow-inner"
                />
              </form>
            </div>
          </div>
