...
export default function AdminMetricCard({ label, value, hint, trend, icon: Icon, color = 'text-indigo-600' }) {
  return (
    <article className="rounded-xl border border-gray-200 bg-white p-3 shadow-sm hover:shadow-md transition-all duration-200 flex flex-col justify-between">
      ...
      <div>
        ...
        <div className="flex items-baseline gap-2">
          <h3 className="text-lg font-black text-gray-900 tracking-tighter">{value}</h3>
          {hint && (
            <p className="text-[9px] font-bold text-gray-400 truncate">{hint}</p>
          )}
        </div>
      </div>
    </article>
