...
import { SOCIAL_LINKS, PERSONAL_INFO, CAREER_OBJECTIVE, EDUCATION } from '@/lib/constants';
...
const Links = () => {
    return (
        <section id="links" className="py-20 px-6 bg-gray-50">
                            ...
                            <div className="space-y-2 text-sm text-gray-600">
                                {EDUCATION.map((edu) => (
                                    <p key={edu.institution}>
                                        {edu.degree} - {edu.institution} ({edu.grade})
                                    </p>
                                ))}
                            </div>
                        </div>
